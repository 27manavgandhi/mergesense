...
const INVARIANTS: Record<InvariantID, InvariantDefinition> = {
  ...
  STATE_AI_INVOCATION_REQUIRES_PENDING: {
    id: 'STATE_AI_INVOCATION_REQUIRES_PENDING',
    description: 'AI can only be invoked when state is AI_REVIEW_PENDING',
    severity: 'fatal',
    evaluate: (ctx: InvariantContext) => {
      if (!ctx.aboutToInvokeAI) return true;
      if (ctx.currentState !== 'AI_REVIEW_PENDING') {
        return false;
      }
      return true;
    },
  ...
  STATE_COMMENT_REQUIRES_REVIEW_READY: {
    id: 'STATE_COMMENT_REQUIRES_REVIEW_READY',
    description: 'Comment can only be posted when state is COMMENT_PENDING',
    severity: 'fatal',
    evaluate: (ctx: InvariantContext) => {
      if (!ctx.aboutToPostComment) return true;
      if (ctx.currentState !== 'COMMENT_PENDING') {
        return false;
      }
      return true;
    },
  ...
  STATE_TERMINAL_NO_FURTHER_TRANSITIONS: {
    id: 'STATE_TERMINAL_NO_FURTHER_TRANSITIONS',
    description: 'Terminal states cannot transition further',
    severity: 'fatal',
    evaluate: (ctx: InvariantContext) => {
      if (!ctx.isTerminalState) return true;
      if (ctx.previousState && ctx.currentState !== ctx.previousState) {
        return false;
      }
      return true;
    },
